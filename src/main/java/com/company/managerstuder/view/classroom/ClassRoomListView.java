package com.company.managerstuder.view.classroom;

import com.company.managerstuder.entity.ClassRoom;
...
@Route(value = "class-rooms", layout = MainView.class)
...
public class ClassRoomListView extends StandardListView<ClassRoom> {
    ...
    @ViewComponent
    private CollectionContainer<ClassRoom> classRoomsDc;

    @ViewComponent
    private InstanceContainer<ClassRoom> classRoomDc;

    @ViewComponent
    private InstanceLoader<ClassRoom> classRoomDl;
    ...
    public void onInit(final InitEvent event) {
        classRoomsDataGrid.getActions().forEach(action -> {
    ...
    @Subscribe("classRoomsDataGrid.createAction")
    public void onClassRoomsDataGridCreateAction(final ActionPerformedEvent event) {
        prepareFormForValidation();

        dataContext.clear();
        ClassRoom entity = dataContext.create(ClassRoom.class);
        classRoomDc.setItem(entity);
        updateControls(true);
    }

    @Subscribe("classRoomsDataGrid.editAction")
    public void onClassRoomsDataGridEditAction(final ActionPerformedEvent event) {
        updateControls(true);
    }
    ...
    @Subscribe(id = "classRoomsDc", target = Target.DATA_CONTAINER)
    public void onClassRoomsDcItemChange(final InstanceContainer.ItemChangeEvent<ClassRoom> event) {
        prepareFormForValidation();

        ClassRoom entity = event.getItem();
        dataContext.clear();
        if (entity != null) {
            classRoomDl.setEntityId(EntityValues.getId(entity));
            classRoomDl.load();
        } else {
            classRoomDl.setEntityId(null);
            classRoomDc.setItem(null);
        }
        updateControls(false);
    }
    ...
    private OperationResult saveEditedEntity() {
        ClassRoom item = classRoomDc.getItem();
    ...
    private ValidationErrors validateView(ClassRoom entity) {
    ...
    private void updateControls(boolean editing) {
        ...
        classRoomsDataGrid.getActions().forEach(Action::refreshState);
