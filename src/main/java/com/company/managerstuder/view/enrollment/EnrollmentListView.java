...
public class EnrollmentListView extends StandardListView<Enrollment> {

    @ViewComponent
    private DataContext dataContext;

    @ViewComponent
    private CollectionContainer<Enrollment> enrollmentsDc;

    @ViewComponent
    private InstanceContainer<Enrollment> enrollmentDc;

    @ViewComponent
    private InstanceLoader<Enrollment> enrollmentDl;

    @ViewComponent
    private VerticalLayout listLayout;

    @ViewComponent
    private DataGrid<Enrollment> enrollmentsDataGrid;
    ...
    public void onInit(final InitEvent event) {
        enrollmentsDataGrid.getActions().forEach(action -> {
    ...
    @Subscribe("enrollmentsDataGrid.createAction")
    public void onEnrollmentsDataGridCreateAction(final ActionPerformedEvent event) {
        prepareFormForValidation();

        dataContext.clear();
        Enrollment entity = dataContext.create(Enrollment.class);
        enrollmentDc.setItem(entity);
        updateControls(true);
    }

    @Subscribe("enrollmentsDataGrid.editAction")
    public void onEnrollmentsDataGridEditAction(final ActionPerformedEvent event) {
        updateControls(true);
    }
    ...
    @Subscribe(id = "enrollmentsDc", target = Target.DATA_CONTAINER)
    public void onEnrollmentsDcItemChange(final InstanceContainer.ItemChangeEvent<Enrollment> event) {
        prepareFormForValidation();

        Enrollment entity = event.getItem();
        dataContext.clear();
        if (entity != null) {
            enrollmentDl.setEntityId(EntityValues.getId(entity));
            enrollmentDl.load();
        } else {
            enrollmentDl.setEntityId(null);
            enrollmentDc.setItem(null);
        }
        updateControls(false);
    }
    ...
    private OperationResult saveEditedEntity() {
        Enrollment item = enrollmentDc.getItem();
        ...
        dataContext.save();
        enrollmentsDc.replaceItem(item);
        updateControls(false);
    ...
    private ValidationErrors validateView(Enrollment entity) {
    ...
    private void updateControls(boolean editing) {
        ...
        enrollmentsDataGrid.getActions().forEach(Action::refreshState);
