package com.company.managerstuder.view.school;

import com.company.managerstuder.entity.School;
...
public class SchoolListView extends StandardListView<School> {
    ...
    @ViewComponent
    private CollectionContainer<School> schoolsDc;

    @ViewComponent
    private InstanceContainer<School> schoolDc;

    @ViewComponent
    private InstanceLoader<School> schoolDl;
    ...
    public void onInit(final InitEvent event) {
        schoolsDataGrid.getActions().forEach(action -> {
    ...
    @Subscribe("schoolsDataGrid.createAction")
    public void onSchoolsDataGridCreateAction(final ActionPerformedEvent event) {
        prepareFormForValidation();

        dataContext.clear();
        School entity = dataContext.create(School.class);
        schoolDc.setItem(entity);
        updateControls(true);
    }

    @Subscribe("schoolsDataGrid.editAction")
    public void onSchoolsDataGridEditAction(final ActionPerformedEvent event) {
        updateControls(true);
    }
    ...
    @Subscribe(id = "schoolsDc", target = Target.DATA_CONTAINER)
    public void onSchoolsDcItemChange(final InstanceContainer.ItemChangeEvent<School> event) {
        prepareFormForValidation();

        School entity = event.getItem();
        dataContext.clear();
        if (entity != null) {
            schoolDl.setEntityId(EntityValues.getId(entity));
            schoolDl.load();
        } else {
            schoolDl.setEntityId(null);
            schoolDc.setItem(null);
        }
        updateControls(false);
    }
    ...
    private ValidationErrors validateView(School entity) {
    ...
    private void updateControls(boolean editing) {
        ...
        schoolsDataGrid.getActions().forEach(Action::refreshState);
