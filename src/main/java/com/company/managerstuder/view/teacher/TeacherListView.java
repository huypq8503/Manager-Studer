package com.company.managerstuder.view.teacher;

import com.company.managerstuder.entity.Teacher;
...
public class TeacherListView extends StandardListView<Teacher> {
    ...
    @ViewComponent
    private CollectionContainer<Teacher> teachersDc;

    @ViewComponent
    private InstanceContainer<Teacher> teacherDc;

    @ViewComponent
    private InstanceLoader<Teacher> teacherDl;
    ...
    public void onInit(final InitEvent event) {
        teachersDataGrid.getActions().forEach(action -> {
    ...
    @Subscribe("teachersDataGrid.createAction")
    public void onTeachersDataGridCreateAction(final ActionPerformedEvent event) {
        prepareFormForValidation();

        dataContext.clear();
        Teacher entity = dataContext.create(Teacher.class);
        teacherDc.setItem(entity);
        updateControls(true);
    }

    @Subscribe("teachersDataGrid.editAction")
    public void onTeachersDataGridEditAction(final ActionPerformedEvent event) {
        updateControls(true);
    }
    ...
    @Subscribe(id = "teachersDc", target = Target.DATA_CONTAINER)
    public void onTeachersDcItemChange(final InstanceContainer.ItemChangeEvent<Teacher> event) {
        prepareFormForValidation();

        Teacher entity = event.getItem();
        dataContext.clear();
        if (entity != null) {
            teacherDl.setEntityId(EntityValues.getId(entity));
            teacherDl.load();
        } else {
            teacherDl.setEntityId(null);
            teacherDc.setItem(null);
        }
        updateControls(false);
    }
    ...
    private OperationResult saveEditedEntity() {
        Teacher item = teacherDc.getItem();
    ...
    private ValidationErrors validateView(Teacher entity) {
    ...
    private void updateControls(boolean editing) {
        ...
        teachersDataGrid.getActions().forEach(Action::refreshState);
